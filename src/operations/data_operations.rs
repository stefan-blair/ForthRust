...
pub fn create(state: &mut evaluate::ForthEvaluator) -> evaluate::ForthResult {
    ...
    let xt = definition::ExecutionToken::ThreadedDefinition(state.memory.top());
...
pub fn constant<N: value::ValueVariant>(state: &mut evaluate::ForthEvaluator) -> evaluate::ForthResult {
    ...
    let xt = definition::ExecutionToken::ThreadedDefinition(address);
    state.definitions.add(word, definition::Definition::new(xt, false));
...
pub fn does(state: &mut evaluate::ForthEvaluator) -> evaluate::ForthResult {
    let object_address = if let definition::ExecutionToken::ThreadedDefinition(address) = state.definitions.get(state.definitions.get_most_recent_nametag()).execution_token {
        address
    } else {
        return Ok(())
    };

    state.memory.write(object_address.plus_cell(1), definition::ExecutionToken::ThreadedDefinition(state.instruction_pointer.unwrap()))?;
